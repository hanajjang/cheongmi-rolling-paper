    ...
    setTimeout(async () => {
      try {
        const dataUrl = await toPng(target, {
          ...
          pixelRatio: 2,
          skipFonts: true,
          // backdrop-filter, blur 등 필터 효과 무시
          filter: (node) => {
            // sticky 헤더 제외하고 캡처
    ...
    <div onClick={(e) => e.target === e.currentTarget && onClose()} style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button onClick={handleSave} disabled={loading} style={{ flex: 2, background: 'linear-gradient(135deg, #e8798a, #d4607a)', color: '#fff', border: 'none', borderRadius: 12, padding: 11, fontSize: 13, fontFamily: 'Nanum Gothic, sans-serif', fontWeight: 700, cursor: 'pointer', boxShadow: '0 3px 12px rgba(212,96,122,0.3)' }}>
            {loading ? '저장 중...' : '💾 저장하기'}
          </button>
        </div>
      </div>
